...
class AllShortlistedStudent extends StatefulWidget {
  const AllShortlistedStudent({super.key});

  @override
  State<AllShortlistedStudent> createState() => _AllShortlistedStudentState();
}

class _AllShortlistedStudentState extends State<AllShortlistedStudent> {
  @override
  void initState() {
    super.initState();
    shortlistedstudent();
 ...
 void shortlistedstudent() async {
   setState(() {
    isLoading = true;
  });
  try {
    var response = await AdminAPI.shortlistedstudent();
 ...
 //search company by yearly 
  ...
  void shortlistedsearch(int year) async {
  if (year==null|| year==0) {
    setState(() {
  ...
  try {
    var response = await AdminAPI.shortlistedyeaarly(year);
    if (response.statusCode == 200) {
      var data = jsonDecode(response.body.toString()) ;
      setState(() {
        filtered=[];
  ...
  Widget build(BuildContext context) {
    ...
    drawer: AdminDrawer(),
      appBar: AppBar(
        title: Text('Shortlisted Students'),
      ),
        ...
        maxLines: 1,
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        Text(company.companyname??'', style: Appconstant.appcardstyle),
                                      ],
                                    ),
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        Text(company.shortlist??'', style: Appconstant.appcardstyle),
                                      ],
                                    ),

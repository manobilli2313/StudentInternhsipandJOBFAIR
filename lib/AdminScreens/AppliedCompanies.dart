...
class AllAppliedCompanies extends StatefulWidget {
  const AllAppliedCompanies({super.key});

  @override
  State<AllAppliedCompanies> createState() => _AllAppliedCompaniesState();
}

class _AllAppliedCompaniesState extends State<AllAppliedCompanies> {
  @override
  void initState() {
    super.initState();
    getallcompanies();
 ...
 void getallcompanies() async {
  ...
  try {
    var response = await AdminAPI.appliedcopanies();
 ...
 //search company by yearly 
  ...
  try {
    var response = await AdminAPI.appliedcompaniesyearly(year);
    if (response.statusCode == 200) {
      var data = jsonDecode(response.body.toString()) ;
      setState(() {
        filtered=[];
    ...
    Flexible(
      ...
      child: Text(  
        ...
        maxLines: 1,
                                   ...
                                   Row(
  mainAxisAlignment: MainAxisAlignment.spaceBetween,
 // crossAxisAlignment: CrossAxisAlignment.start, // allow vertical alignment
  children: [

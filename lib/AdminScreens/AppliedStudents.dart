...
class AllAppliedStudents extends StatefulWidget {
  const AllAppliedStudents({super.key});

  @override
  State<AllAppliedStudents> createState() => _AllAppliedStudentsState();
}

class _AllAppliedStudentsState extends State<AllAppliedStudents> {
  @override
  void initState() {
    super.initState();
    getallappliedstudents();
  }
  List<appliedStudent> clist=[];
  List<appliedStudent> filtered=[];
  TextEditingController searchController = TextEditingController();
  bool isLoading = false;

 void getallappliedstudents() async {
   setState(() {
    isLoading = true;
  });
  try {
    var response = await AdminAPI.appliedstudents();
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        for (var i in data['Data']) {
          clist.add(appliedStudent.fromJson(i));
        }
 ...
 //search company by yearly 
  ...
  try {
    var response = await AdminAPI.appliedstudentyearly(year);
    if (response.statusCode == 200) {
      var data = jsonDecode(response.body.toString()) ;
      setState(() {
        filtered=[];
       // clist=[];
        for(var i in data['Data']){
            filtered.add(appliedStudent.fromJson(i));
          ...
          Expanded(
            child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          appliedStudent company = filtered[index];
                          return SizedBox(
